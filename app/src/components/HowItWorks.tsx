  ...
  return (
    <section id="how-it-works" className="py-20 md:py-28 relative overflow-hidden">
      ...
      <div className="section-pad relative z-10" ref={ref}>
        ...
        >
          <span className="inline-block px-3 py-1 rounded-full bg-brand-50 text-brand-600 text-xs font-bold tracking-wider uppercase mb-4">
            How it works
          </span>
          ...
          <p className="text-warm-800/50 text-base md:text-lg max-w-md mx-auto">
            No spreadsheets. No guilt trips. Just clarity.
          </p>
        </motion.div>

        {/* Swipeable carousel */}
        <div className="relative">
          {/* Cards container — horizontal scroll on mobile, grid on larger */}
            ...
            {howItWorksSteps.map((step, i) => (
              ...
              >
                <div className={`h-full rounded-3xl bg-gradient-to-br ${step.color} border ${step.border} p-6 flex flex-col transition-shadow hover:shadow-card-hover`}>
                  ...
                  {/* Step number */}
                  <span className="text-xs font-bold text-warm-800/30 tracking-wider uppercase mb-2">
                    Step {step.number}
                  </span>

                  {/* Title */}
                  <h3 className="font-display font-700 text-lg text-warm-900 tracking-tight mb-2">
                    {step.title}
                  </h3>

                  {/* Description */}
                  <p className="text-warm-800/55 text-sm leading-relaxed flex-1">
                    {step.description}
                  </p>
                </div>
              </motion.div>
